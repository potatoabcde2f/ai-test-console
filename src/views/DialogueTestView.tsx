...
export function DialogueTestView({
  ...
  externalLoading,
  setExternalLoading,
  externalPendingImages: _externalPendingImages,
  setExternalPendingImages: _setExternalPendingImages,
...
根据下方提供的对话内容，模拟用户接下来最可能主动提出的 3 个穿搭问题，需要围绕当前的话题，不能突兀。不一定非是要问题类型的，也可以是"可以再推荐一套类似的吗？"、"把这套穿搭生成图片"等等这种请求型的。问题需要尽可能覆盖全面，适用大多数人。出问题时可以根据AI的最后一句进行延伸拓展，比如AI说"如果你愿意，我也可以按你的**西装颜色**，直接帮你配出一整套。"可以出题为"白色西装，帮我搭一整套"；如果AI说"要不要我顺便帮你配一套"灰西装+包+鞋子"的完整搭配？"，你可以出题为"好的，帮我搭一套吧"，承接AI的最后一句，作为用户的回答
